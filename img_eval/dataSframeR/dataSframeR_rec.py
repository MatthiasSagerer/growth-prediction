...
dataSframeR_workb_path = 'C:/Users/Matthias Sagerer/Documents/TUM/bachelor_thesis/growth-prediction/results_xlsx/dataSframeR/dataSframeR_recall.xlsx'
    ...
    for res_nr, cur_res_dirs in enumerate(cur_dataS_dirs):
        ...
        for vid_nr, (vid_gt, vid_pred) in enumerate(zip(vids_gt, vids_pred)):
            ...
            for frame, (fr_gt, fr_pred) in enumerate(zip(frs_gt[6:], frs_pred[6:])):
                ...
                metric_score = calRecall(gt_fr, pred_fr)

                # append metric score to video and dataset list
