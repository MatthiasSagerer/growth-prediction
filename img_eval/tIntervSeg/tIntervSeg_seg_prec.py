...
seg_workb_path = 'C:/Users/Matthias Sagerer/Documents/TUM/bachelor_thesis/growth-prediction/results_xlsx/tIntervSeg_seg_precision.xlsx'
...
for dir_nr, dirs in enumerate(seg_dirs):
    ...
    # create empty list for metric scores
    ...
    for j, (vid_gt, vid_pred) in enumerate(zip(vids_gt, vids_pred)):
        ...
        for frame, (fr_gt, fr_pred) in enumerate(zip(frs_gt[6:], frs_pred[6:]), 7):
            ...
            metric = calPrecision(cur_gt_fr, cur_pred_fr)
        ...
        print(f'Seg metric progress: {progress} %')
...
sum_worksh.write('A11', 'Total avg precision')
