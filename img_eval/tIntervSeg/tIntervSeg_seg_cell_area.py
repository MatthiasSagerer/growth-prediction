...
seg_workb_path = 'C:/Users/Matthias Sagerer/Documents/TUM/bachelor_thesis/growth-prediction/results_xlsx/tIntervSeg_seg_cell_area.xlsx'
...
# create empty lists for cell area means/standard deviation
cell_area_means = []
cell_area_stds = []
...
for dir_nr, dirs in enumerate(seg_dirs):
    ...
    # create empty list for cell areas
    for frame in range(1, 7):
        exec(f"fr{frame}_cell_areas = []")
    ...
    for j, (vid_gt, vid_pred) in enumerate(zip(vids_gt, vids_pred)):
        ...
        for frame, (fr_gt, fr_pred) in enumerate(zip(frs_gt[6:], frs_pred[6:]), 7):
            ...
            # read gt/pred img and calculate cell area
            ...
            cell_area = calCellArea(cur_gt_fr, cur_pred_fr)

            # append cell area to list and write to excel sheet
            exec(f"fr{frame-6}_cell_areas.append({cell_area})")
            exec(
                f"{dir_type}_worksh.write_number('{chr(66+frame-7)}{j+2}', {cell_area})")

        # print progress to cli
        progress = round((dir_nr*83+j+1)/(len(seg_dirs)*83)*100, 2)
        print(f'Seg cell area progress: {progress} %')

    dir_cell_area_means = []
    dir_cell_area_stds = []

    # calculate cell area mean and standard deviation for frames and write them to excel sheet
    for fr_nr in range(1, 7):
        exec(f"fr{fr_nr}_cell_areas_arr = np.array(fr{fr_nr}_cell_areas)")
        exec(f"cell_area_mean = fr{fr_nr}_cell_areas_arr.mean()")
        exec(f"cell_area_std = fr{fr_nr}_cell_areas_arr.std()")
        dir_cell_area_means.append(cell_area_mean)
        dir_cell_area_stds.append(cell_area_std)
        cell_area_means.append(cell_area_mean)
        cell_area_stds.append(cell_area_std)
        cell_area_mean = round(cell_area_mean, 6)
        cell_area_std = round(cell_area_std, 6)
        exec(
            f"sum_worksh.write_number('{chr(65+2*(dir_nr+1)-1)}{fr_nr+2}', {cell_area_mean})")
        exec(
            f"sum_worksh.write_number('{chr(65+2*(dir_nr+1))}{fr_nr+2}', {cell_area_std})")

    # calculate directory cell area and standard deviation means and write to excel file
    dir_cell_area_means_arr = np.array(dir_cell_area_means)
    dir_cell_area_stds_arr = np.array(dir_cell_area_stds)
    dir_mean = dir_cell_area_means_arr.mean()
    dir_mean_std = dir_cell_area_stds_arr.mean()
...
# calculate total cell area mean and total mean cell area deviation and write to excel file
cell_area_means_arr = np.array(cell_area_means)
cell_area_stds_arr = np.array(cell_area_stds)
total_mean = cell_area_means_arr.mean()
total_mean_std = cell_area_stds_arr.mean()
total_mean = round(total_mean, 6)
total_mean_std = round(total_mean_std, 6)
sum_worksh.write('A11', 'Total avg cell area')
