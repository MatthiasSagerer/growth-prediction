...
# 2 12 FrVi per cutout => 182 vids
# 21 test vids, 161 train vids
...
left_well = tiff.imread(os.path.join(
    source_dir, '07_left_well_auto_threshold.tif'))
...
for h in range(fr_series):    # frames
    for i in range(cols):      # cols
        for j in range(rows):   # rows
            ...
            current_tif = left_well[36*h:36*(h+1), y_offset+512 *
                                    j:y_offset+512*(j+1), x_offset+512*i:x_offset+512*(i+1)]
