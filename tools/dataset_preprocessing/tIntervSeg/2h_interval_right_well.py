...
overview_file_path = os.path.join(dir, '2h_overview_right_well.txt')
with open(overview_file_path, 'w') as file:
    file.write('File overview for 2h right well\n')
    ...
    for i in range(cols):      # columns
        for j in range(rows):   # rows
            if (i == 5 and j == 8) or (i == 6 and j == 7) or (i == 6 and j == 8 and h == 0):
                ...
                current_tif = right_well[24*h:24*(h+1), x_offset+512*i:x_offset+512*(
