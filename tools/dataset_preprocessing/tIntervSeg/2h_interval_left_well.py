...
# original
# dir = os.path.join(target_dir, 'ori_2h')

# segmented
# dir = os.path.join(target_dir, 'seg_2h')

# find edges
# dir = os.path.join(target_dir, 'edg_2h')

# create test/train folder and overview file
os.mkdir(os.path.join(dir, 'train'))
os.mkdir(os.path.join(dir, 'test'))
overview_file_path = os.path.join(dir, '2h_overview_left_well.txt')
with open(overview_file_path, 'w') as file:
    file.write('File overview for 2h left well\n')
...
for h in range(fr_series):    # frames
    for i in range(cols):      # columns
        for j in range(rows):   # rows
            if (i == 5 and j == 8) or (i == 6 and j == 7) or (i == 6 and j == 8 and h == 0):
                # these frames get skipped to have a consistent size for all data sets
                print(
                    f'frames {24*h+1}-{24*(h+1)}, column {i}, row {j} are skipped')
                with open(overview_file_path, 'a') as file:
                    file.write(
                        f'SKIPPED: column{i}, row{j}, frames {24*h+1}-{24*(h+1)}\n')
            else:
                # 'test_set' and 'subdir' are used to split into test/train set
                subdir = ''
                test_set = (i == 0 and j == 0) or (i == 1 and j == 1) or (i == 2 and j == 2) or (
                    i == 3 and j == 3) or (i == 4 and j == 4) or (i == 5 and j == 5) or (i == 6 and j == 6)
                if test_set:
                    subdir = 'test'
                else:
                    subdir = 'train'
                # select the current_tif
                current_tif = left_well[24*h:24*(h+1), x_offset+512*i:x_offset+512*(
                ...
                with open(overview_file_path, 'a') as file:
                    file.write(
                        f'video_{video_num_str} (ori) in {subdir}: column{i}, row{j}, frames {24*h+1}-{24*(h+1)}\n')
                ...
                with open(overview_file_path, 'a') as file:
                    file.write(
                        f'video_{video_num_str} (rot) in {subdir}: column{i}, row{j}, frames {24*h+1}-{24*(h+1)}\n')
                print(f'{subdir} video_{video_num_str} saved.')
