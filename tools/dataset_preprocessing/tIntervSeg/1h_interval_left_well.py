...
overview_file_path = os.path.join(dir, '1h_overview_left_well.txt')
with open(overview_file_path, 'w') as file:
    file.write('File overview for 1h left well\n')
    ...
    for i in range(cols):      # columns
        for j in range(rows):   # rows
            if (i == 3 and j == 5) or ((h == 0 or h == 1 or h == 4 or h == 5) and j == 7 and i == 3):
                ...
                current_tif = left_well[12*h:12*(h+1), x_offset+512*i:x_offset+512*(
...
# Steps to adjust code:
# - change all "right"/"left" to "left"/"right"
# - adjust the time interval: 1h/2h/3h/4h
# - adjust time series according to the frames: 12/24/36/48
# - adjust step size according to the time series: 1, 2, 3, 4
# - adjust conditions for omitted files
# - adjust conditions for test set
# - adjust video numbers (test: 0/42; train: 0/322)
# - (un-)comment creation of test/train dir

'''
Optional
TODO: save details about which frame series, columns and rows were selected automatically
'''
