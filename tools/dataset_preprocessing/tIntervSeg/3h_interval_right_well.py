...
right_well = tiff.imread(os.path.join(
    source_dir, '04_right_well_find_edges_72fr.tif'))
...
overview_file_path = os.path.join(dir, '3h_overview_right_well.txt')
with open(overview_file_path, 'w') as file:
    file.write('File overview for 3h right well\n')
    ...
    for i in range(cols):      # columns
        for j in range(rows):   # rows
            if False:
                ...
                current_tif = right_well[36*h:36*(h+1), x_offset+512*i:x_offset+512*(
