...
overview_file_path = os.path.join(dir, '4h_overview_right_well.txt')
with open(overview_file_path, 'w') as file:
    file.write('File overview for 4h right well\n')
...
for h in range(fr_series):    # frames
    for i in range(cols):      # cols
        for j in range(rows):   # rows
            ...
            current_tif = right_well[fr_range[0]:fr_range[1], y_offset +
